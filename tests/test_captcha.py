...
class CaptchaTestCaseMixin:
    ...
    def test_register_user_require_captcha_validation(self):
        try:
            res = self.make_request(self.register_query_without_cap_fields(username='fdsafsdfgv'))
    ...
    def test_login_require_captcha_validation(self):
        try:
            res = self.make_request(self.login_query_without_cap_fields(username='fake'))
        except Exception as e:
    ...
    def test_register_wrong_captcha_validation(self):
        ...
        res = self.make_request(
            self.register_query(uuid=cap.uuid)
        )
    ...
    def test_register_wrong_uuid(self):
        cap = self.gen_captcha()
        res = self.make_request(
            self.register_query(uuid=uuid4(), input_=cap.text)
        )
    ...
    def test_register_correct_captcha_create_user(self, username="test_captcha"):
        ...
        cap = self.gen_captcha()
        res = self.make_request(
            self.register_query(username=username, password="SuperSecureP@ssw0rd", uuid=cap.uuid, input_=cap.text)
        )
